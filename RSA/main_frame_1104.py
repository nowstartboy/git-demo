...
class WorkerThread(threading.Thread):     #画实时监测信号动态图  用于月报监测和自定义监测功能模块中
	...
	def run(self):#运行一个线程
		...
		str_time = str(datetime.datetime.now().strftime('%F-%H-%M-%S'))  # 整个测试的起始的准确时间用于存储原始文件路径的因为路径中不能有冒号
		...
		while time.time() - time_ref < t and self._running:
			# 获取平均平均噪声电平，去频带内迹线最低点+15dB范围内信号的平均值
			...
			spectrum1 = method.spectrum1(self.rsa_true,average, startFreq, stopFreq, span, self.rbw, 
										self.vbw, str_time, count, str_tt1, str_tt2,longitude_set,latitude_set,num_signal,Sub_cf_all,threshold)
			# 若正常返回扫频数据，则进行显示，处理与存储
			if spectrum1 != 0:
				# 返回信息：频率列表，电平列表，扫描中心频率数组，扫描span数组，信号段中心频率数组，信号段检测带宽数组，信号段峰值数组，完整扫频频率数据，
				# 完整扫频电平数据，完整迹线频率，完整迹线电平，红框四角，比红框宽一些的范围，检测到信号数量，信号段是否合法标识数组，业务名数组
				head,data1,Sub_cf_channel,Sub_span,Sub_cf,Sub_band,Sub_peak,Sub_Spectrum1,Sub_Spectrum2,\
					freq1,traceData1,point,point_xy,num_signal_new,Sub_illegal,Sub_type,ServiceDID= spectrum1
				num_signal=num_signal_new
				#把不重复信号给滤除掉
				Sub_cf_length = len(Sub_cf_all) #之前扫频已发现存在信号
				# 若存在发射信号
				if Sub_cf:
					if not Sub_cf_all:    # 首次发现信号
						Sub_cf_all=Sub_cf[:]
						...
						for i in range(len(Sub_cf)):
							...
							self.window.list_ctrl.SetItem(i,4,str(Sub_type[i]))   #业务类型为Freq_Name
							#如果判定为不合格则标注为红色
							if Sub_illegal[i]==0:
								self.window.list_ctrl.SetItemBackgroundColour(i, "red")
					else:   # 非首次发现信号
						for cf_i in range(len(Sub_cf)):   #对新发现的信号进行重复性判断
							divide_cf = array(Sub_cf_all)-Sub_cf[cf_i]   #求已发现信号与新发现信号中心频率的差值
							#判断是否有重复信号（cf在200KHz以内为重复），如果有重复，则给出是与第几个列表信号重复
							divide_no = 0 
							...
							#如果重复了，则与那个重复信号幅度做比较，如果比之前的大，则替换掉之前的信号幅度
							else:
...
#					print (count)
				...
				if time.time()-load_time>35 :  
					...
					# 取测试数据统计后写入月报数据库
					method.rmbt_facility_freqband_emenv(str_time,self.span,load_time_pre,time_now,longitude_set,
														latitude_set,ServiceDID,threshold)
					# 取测试数据统计后写入月报数据库
					method.rmbt_facility_freq_emenv3(str_time,load_time_pre,time_now,'01',ServiceDID,threshold) #ssid默认为'01'
					# 取所有迹线测试数据，统计每个扫频频点的占用度，按月报占用度数据表要求写入
					method.rmbt_freq_occupancy(self.span,load_time_pre,time_now,self.start_freq,self.end_freq,longitude_set,
											latitude_set,height_set,trace2,head[3:],average,threshold)
					load_time=time.time()
		...
		if mon_or_real:   # true 时是自定义监测
			...
			self.window.m_textCtrl_midFreq.Enable(True)
			self.window.m_textCtrl_span.Enable(True)
			self.window.m_tc_MonitorTime.Enable(True)
			self.window.m_textCtrl_midFreq.SetValue('')
			self.window.m_textCtrl_span.SetValue('')
		else:
		...
		'''  commented by vinson by 20181016; remove the function of raw data store; 
		...
		path_name = method.config_data['Spec_dir']+"\\%s\\"%(str_time+'--'+str_time1 + ("spectrum%d" % t))
		file_path = method.config_data['Spec_dir']+"\\%s\\"%(str_time+'--'+str_time1 + ("spectrum%d" % t)) + (str_time+'--'+str_time1) + "Max_spectrum%ds.csv" % t  # 频谱数据粗扫描数据存储路径
		...
		with con:
			...
			cur.execute("INSERT INTO Minitor_Task VALUES('%s',to_date('%s','yyyy-MM-dd hh24:mi:ss'),to_date('%s',\
						'yyyy-MM-dd hh24:mi:ss'),%s,%s,'%s','%s','%s',%s,%s,%s,%s,%s)"%(str_time, start_time, str_time1, 
						float(t), float(s_c), file_path, deviceSerial, anteid, count,float(startFreq.value), float(stopFreq.value),
						float(longitude_set),float(latitude_set)))
			cur.close()
		con.commit()
		con.close()
...
class Wait_GPS (wx.Dialog):
	def __init__( self, parent,title=u"GPS信息获取" ):
		...
		self.button1 = wx.Button( self, wx.ID_OK, "确认", wx.DefaultPosition, wx.DefaultSize, 0 )
		self.button2 = wx.Button( self, wx.ID_ANY, "取消", wx.DefaultPosition, wx.DefaultSize, 0 )
...
class MyPanel1_1 ( wx.Panel ):
	...
	def __init__( self, parent ):
		...
		# set midFreq
		bSizer_midFreq = wx.BoxSizer( wx.HORIZONTAL )
		self.m_staticText_midFreq = wx.StaticText( self.panel2, wx.ID_ANY, u"中心频率： ", wx.DefaultPosition, wx.DefaultSize, 0 )
		#self.m_staticText3.Wrap( -1 )
		#self.m_staticText3.SetFont( wx.Font( 12, 70, 90, wx.BOLD, False, "宋体" ) )
		bSizer_midFreq.Add( self.m_staticText_midFreq, 0, wx.ALL|wx.ALIGN_CENTRE, 5 )
		
		default_mid_freq = ''
		self.m_textCtrl_midFreq = wx.TextCtrl( self.panel2, wx.ID_ANY, default_mid_freq, wx.DefaultPosition, (80,20) , 0 )
		#self.Bind(wx.EVT_TEXT_ENTER, self.change_midFreq, self.m_textCtrl1)
		#self.m_textCtrl_midFreq.Bind(wx.EVT_KILL_FOCUS,self.change_midFreq)
		bSizer_midFreq.Add( self.m_textCtrl_midFreq, 0, wx.ALL|wx.ALIGN_CENTRE, 0 )
		
		m_choice1Choices = ['KHz','MHz','GHz']
		self.m_choice_midFreq = wx.Choice( self.panel2, wx.ID_ANY, wx.DefaultPosition, (50,20), m_choice1Choices, 0 )
		self.m_choice_midFreq.SetSelection( 1 )
		bSizer_midFreq.Add( self.m_choice_midFreq, 0, wx.ALL|wx.ALIGN_CENTRE, 2 )
		self.m_choice_midFreq.Bind(wx.EVT_CHOICE,self.OnChangeStartFreUnit)
		bSizer11.Add( bSizer_midFreq, 1, wx.EXPAND, 0 )
		...
		bSizer11.Add( bSizer29, 1, wx.EXPAND, 0 )
		...
		bSizer27.Add( bSizer14, 1, wx.EXPAND, 0 )
		
		# set MonitorSpan
		bSizer_span = wx.BoxSizer( wx.HORIZONTAL )
		self.m_staticText_span = wx.StaticText( self.panel2, wx.ID_ANY, u" SPAN  ：", wx.DefaultPosition, wx.DefaultSize, 0 )
		#self.m_staticText3.Wrap( -1 )
		#self.m_staticText3.SetFont( wx.Font( 12, 70, 90, wx.BOLD, False, "宋体" ) )
		bSizer_span.Add( self.m_staticText_span, 0, wx.ALL|wx.ALIGN_CENTRE, 5 )
		
		default_span = ''
		self.m_textCtrl_span = wx.TextCtrl( self.panel2, wx.ID_ANY, default_span, wx.DefaultPosition, (80,20) , wx.TE_PROCESS_ENTER )
		#self.Bind(wx.EVT_TEXT_ENTER, self.change_span, self.m_textCtrl1)
		#self.m_textCtrl_span.Bind(wx.EVT_KILL_FOCUS,self.change_span)
		bSizer_span.Add( self.m_textCtrl_span, 0, wx.ALL|wx.ALIGN_CENTRE, 0 )
		
		m_choice1Choices = ['KHz','MHz','GHz']
		self.m_choice_span = wx.Choice( self.panel2, wx.ID_ANY, wx.DefaultPosition, (50,20), m_choice1Choices, 0 )
		self.m_choice_span.SetSelection( 1 )
		bSizer_span.Add( self.m_choice_span, 0, wx.ALL|wx.ALIGN_CENTRE, 2 )
		self.m_choice_span.Bind(wx.EVT_CHOICE,self.OnChangeStartFreUnit)
		bSizer27.Add( bSizer_span, 1, wx.EXPAND, 0 )
		...
		self.axes_score.set_title('自由监测频谱图')
		self.axes_score.grid(True,color='dimgray')
		self.axes_score.set_xlabel('频率/Hz')
		self.axes_score.set_ylabel('幅度/dBmV')
		self.canvas.draw()
		...
		# Box-storaging monitor spectrum
		self.bSizer19 = wx.BoxSizer( wx.VERTICAL )
		...
		self.bx1.SetFont(font)
		self.sbSizer35 = wx.StaticBoxSizer( self.bx1, wx.VERTICAL )
		self.list_ctrl = wx.ListCtrl(self, size=(-1,220),style=wx.LC_REPORT|wx.LC_HRULES|wx.LC_VRULES|wx.BORDER_SUNKEN)
		self.list_ctrl.InsertColumn(0, 'ID',width=30)
		self.list_ctrl.InsertColumn(1, '频率/MHz',width=75)
		self.list_ctrl.InsertColumn(2, '带宽/MHz', width=80)
		self.list_ctrl.InsertColumn(3, '幅度/dBmV',width = 80)
		self.list_ctrl.InsertColumn(4, '业务类型', width=65)
		self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.OnClick_list, self.list_ctrl)
		self.Bind(wx.EVT_LIST_ITEM_RIGHT_CLICK, self.RightClick_list, self.list_ctrl)
		...
		# Box-storaging pre-obeserved specturm
		self.bSizer_pre = wx.BoxSizer( wx.VERTICAL )
		
		self.bx_pre=wx.StaticBox( self, wx.ID_ANY, u"待观察信号列表：" )
		#bx_pre.SetBackgroundColour("MEDIUM GOLDENROD")
		self.bx_pre.SetForegroundColour("SLATE BLUE")
		font_pre = wx.Font(10, wx.DECORATIVE,wx.NORMAL,wx.BOLD)
		self.bx_pre.SetFont(font_pre)
		self.sbSizer_pre = wx.StaticBoxSizer( self.bx_pre, wx.VERTICAL )
		self.list_ctrl_pre = wx.ListCtrl(self, size=(-1,115),style=wx.LC_REPORT|wx.LC_HRULES|wx.LC_VRULES|wx.BORDER_SUNKEN)
		#self.list_ctrl_pre.InsertColumn(0, 'ID',width=30)
		self.list_ctrl_pre.InsertColumn(1, '频率/MHz',width=75)
		self.list_ctrl_pre.InsertColumn(2, '带宽/MHz', width=90)
		self.list_ctrl_pre.InsertColumn(3, '幅度/dBmV',width = 90)
		self.list_ctrl_pre.InsertColumn(4, '业务类型', width=65)
		self.Bind(wx.EVT_LIST_ITEM_ACTIVATED, self.OnClick_list_pre, self.list_ctrl_pre)
		self.Bind(wx.EVT_LIST_ITEM_RIGHT_CLICK, self.RightClick_list_pre, self.list_ctrl_pre)
		
		self.sbSizer_pre.Add(self.list_ctrl_pre,0,wx.ALL|wx.EXPAND,0)
		
		self.bSizer_pre.Add(self.sbSizer_pre,0,wx.ALL|wx.EXPAND,5)
		
		bSizer6.Add( self.bSizer_pre, 1, wx.EXPAND, 5 )
				...
				if state == True:
					if detection_state==0:
						try:
							...
							# test midFreq and span input
							mid_freq_string = self.m_textCtrl_midFreq.GetValue()
							self.midFreq_unit = self.m_choice_midFreq.GetString(self.m_choice_midFreq.GetSelection())
							mid_freq = method.test_textCtrl_input(mid_freq_string, self.midFreq_unit)
							
							span_string = self.m_textCtrl_span.GetValue()
							self.span_unit = self.m_choice_span.GetString(self.m_choice_span.GetSelection())
							span_freq = method.test_textCtrl_input(span_string, self.span_unit)
							
							if mid_freq > 0 and span_freq > 0 and mid_freq > span_freq/2 + 9e-3 and mid_freq < 6.2e3-span_freq/2:
								start_freq = (mid_freq - span_freq/2) * 1e6
								self.start_freq_unit=self.m_choice1.GetString(self.m_choice1.GetSelection())
								if self.start_freq_unit == 'KHz':
									self.m_textCtrl1.SetValue(str(start_freq/1e3))
								elif self.start_freq_unit == 'MHz':
									self.m_textCtrl1.SetValue(str(start_freq/1e6))
								else:
									self.m_textCtrl1.SetValue(str(start_freq/1e9))
								
								end_freq = (mid_freq + span_freq/2) * 1e6
								self.end_freq_unit=self.m_choice2.GetString(self.m_choice2.GetSelection())
								if self.end_freq_unit == 'KHz':
									self.m_textCtrl2.SetValue(str(end_freq/1e3))
								elif self.end_freq_unit == 'MHz':
									self.m_textCtrl2.SetValue(str(end_freq/1e6))
								else:
								...
								start_freq_string=self.m_textCtrl1.GetValue()
								self.m_choice1_string=self.m_choice1.GetString(self.m_choice1.GetSelection())
								start_freq = method.test_textCtrl_input(start_freq_string, self.m_choice1_string)
								if start_freq < 0:
									start_freq=config_data['start_freq']
									wx.MessageBox(u' 初始频率输入须为非负数值', "Message" ,wx.OK | wx.ICON_INFORMATION)
								if start_freq>6.2e3:
								...
								end_freq=self.m_textCtrl2.GetValue()
								self.m_choice2_string=self.m_choice2.GetString(self.m_choice2.GetSelection())
								print ('end_input success')
								end_freq = method.test_textCtrl_input(end_freq, self.m_choice2_string)
								if end_freq < 0:
									end_freq=config_data['end_freq']
									wx.MessageBox(u' 终止频率输入须为非负数值', "Message" ,wx.OK | wx.ICON_INFORMATION)
								if end_freq<9e-3:
							...
							self.m_textCtrl_midFreq.Enable(False)
							self.m_textCtrl_span.Enable(False)
							...
							self.m_textCtrl2.Enable(True)
							self.m_textCtrl3.Enable(True)
							self.m_textCtrl_midFreq.Enable(True)
							self.m_textCtrl_span.Enable(True)
							self.m_tc_MonitorTime.Enable(True)
							self.m_textCtrl_midFreq.SetValue('')
							self.m_textCtrl_span.SetValue('')
					else:
						event.GetEventObject().SetLabel(u"开始监测")
						event.GetEventObject().SetValue(False)
	...
	def RightClick_list(self, event):
		list_select_cf = self.list_ctrl.GetItemText(event.GetIndex(),1)
		list_select_band = self.list_ctrl.GetItemText(event.GetIndex(),2)
		list_select_peak = self.list_ctrl.GetItemText(event.GetIndex(),3)
		list_select_type = self.list_ctrl.GetItemText(event.GetIndex(),4)
		row_insert = self.listCtrlPreNum
		self.list_ctrl_pre.InsertItem(row_insert,list_select_cf)
		self.list_ctrl_pre.SetItem(row_insert,1,list_select_band)
		self.list_ctrl_pre.SetItem(row_insert,2,list_select_peak)
		self.list_ctrl_pre.SetItem(row_insert,3,list_select_type)   
		self.listCtrlPreNum += 1
	
	def OnClick_list_pre(self, event):
		try:
			list_select_cf = self.list_ctrl_pre.GetItemText(event.GetIndex(),0)
			list_select_band = float(self.list_ctrl_pre.GetItemText(event.GetIndex(),1))
		except:
			return
		
		self.m_textCtrl_midFreq.SetValue(list_select_cf)
		if list_select_band * 10 > 3:
			list_select_band = list_select_band * 10
		else:
			list_select_band = 3
		self.m_textCtrl_span.SetValue('%.3f' % (list_select_band))
		
	
	def RightClick_list_pre(self, event):
		self.list_ctrl_pre.DeleteItem(event.GetIndex())
		self.listCtrlPreNum -= 1
...
class MyPanel1_2 ( wx.Panel ):
	...
	def __init__( self, parent ):
		...
		#self.axes_score.axhline(y=average, color='r')
		self.axes_score.set_ylim(-80,0)
		self.axes_score.set_title(u"月报监测频谱图")
		self.axes_score.grid(True,color='dimgray')
		self.axes_score.set_xlabel('频率/Hz')
		self.axes_score.set_ylabel('幅度/dBmVV')
		self.canvas.draw()
...
class MyPanel3 ( wx.Panel ):
	...
	def __init__( self, parent ):
		...
		self.axes_score4.set_title('信道占用度')
		self.axes_score4.grid(True,color='y')
		self.axes_score4.set_xlabel('频率/Hz')
		self.axes_score4.set_ylabel('百分比/%')
		...
		self.axes_score5.set_title('频谱占用度')
		self.axes_score5.grid(True,color='y')
		self.axes_score5.set_xlabel('时间/小时')
		self.axes_score5.set_ylabel('百分比/%')
	...
	def channel_occ(self,start_time,stop_time,task_name,freq_start,freq_stop):
		...
		if not b.empty:
			print('not empty in channel_occ')
			channel_occupied = []
			for i in range(20):
				...
				channel_occupied1 = len(a) / float(b['COUNT'][0])
	...
	def spectrum_occ(self,start_time,stop_time,task_name,freq_start,freq_stop):
		...
		sql4 = "select COUNT from minitor_task where Task_Name='%s'" % (task_name)
		#con = mdb.connect('localhost', 'root', 'cdk120803', 'ceshi1')
		sql_count = pandas.read_sql(sql4, con)
		#print (c)
		con.commit()
		con.close()
...
class MyPanel4 ( wx.Panel ):
	...
	def __init__( self, parent ):
		...
		self.axesLeft.set_title('测向频谱图')
		self.axesLeft.grid(True,color='dimgray')
		self.axesLeft.set_xlabel('频率/Hz')
		self.axesLeft.set_ylabel('幅度/dBmV')
...
class MyPanel5 ( wx.Panel ):
	...
	def __init__( self, parent ):
		...
		self.axes_score.set_title('无人机频谱图')
		self.axes_score.grid(True,color='dimgray')
		...
		self.axes_score_new.set_title('无人机监测瀑布图')
		self.axes_score_new.grid(True,color='dimgray')
		self.axes_score_new.set_xlabel('频率/Hz')
...
class MyDialog1 ( wx.Dialog ):
	...
	def __init__( self, parent,title=u"默认配置" ):
		...
		self.m_dirPicker2 = wx.DirPickerCtrl( self, wx.ID_ANY, config_data['Spec_dir'], u"选择数据文件夹", wx.DefaultPosition, wx.DefaultSize, wx.DIRP_DEFAULT_STYLE)
		bSizer181.Add( self.m_dirPicker2, 0, wx.ALL|wx.EXPAND, 5 )

		
		self.m_filepath = wx.FilePickerCtrl( self, wx.ID_ANY, config_data['IQ_dir'], u"选择文件", u"", wx.DefaultPosition, wx.DefaultSize, wx.FLP_DEFAULT_STYLE )
		...
		self.m_staticText105 = wx.StaticText( self, wx.ID_ANY, u"台站数据库密码: ", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_staticText88 = wx.StaticText( self, wx.ID_ANY, u"本地: ", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_staticText102 = wx.StaticText( self, wx.ID_ANY, u"本地数据库密码: ", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_staticText101 = wx.StaticText( self, wx.ID_ANY, u"月报: ", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_staticText103 = wx.StaticText( self, wx.ID_ANY, u"月报数据库密码: ", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_button44 = wx.Button( self, wx.ID_OK, "确认", wx.DefaultPosition, wx.DefaultSize, 0 )
		...
		self.m_button45 = wx.Button( self, wx.ID_CANCEL, u"取消", wx.DefaultPosition, wx.DefaultSize, 0 )
...
#				self.m_textCtrl51.SetValue(self.message[1])
	...
	def select_Config(self,event):
		...
		if a.ShowModal()==wx.ID_OK:
			...
			# except (ValueError,TypeError) as e:
				...
				# return	
			try:
				config_data['trigger_level']=float(value[5])    # 信号检测阈值
				threshold = float(value[5])
			except (ValueError,TypeError) as e:
	...
	def get_help_txt(self,event):
		# print ('get help txt')
		filepath = 'file:///' + os.getcwd() + '/help/help.html'
